...
class StepPrepareData:
    ...
    @staticmethod
    def process_row(row_count, id_row, path_series):
        ...
        if path_child.exists():
            ...
            return row_count, data_series_row.values.reshape((1, -1))
        else:
    ...
    def get_partition_prepared(self, path: str, is_train: bool) -> pd.DataFrame:
        ...
        results = Parallel(n_jobs=-1)(
            delayed(self.process_row)(row_count, id_row, path_series)
            for row_count, id_row in tqdm(enumerate(ids), total=len(ids))
        )

        # Aggregate the results
        for row_count, data_series_row in results:
